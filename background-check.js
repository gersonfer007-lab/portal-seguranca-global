...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ===== AUTO-FILL: se veio redirecionado da pagina principal =====
  var urlParams = new URLSearchParams(window.location.search);
  var docParam = urlParams.get('doc');
  var typeParam = urlParams.get('type');
  if (docParam && typeParam) {
    // Seleciona a aba correta (CPF/CNPJ/Nome)
    if (['cpf', 'cnpj', 'nome'].indexOf(typeParam) !== -1) {
    ...
    if (typeParam === 'nome') {
      ...
      if (nameInput) nameInput.value = decodeURIComponent(docParam);
    } else {
      var docInput = document.getElementById('doc-input');
      if (docInput) docInput.value = decodeURIComponent(docParam);
    }

    // Limpa a URL para nao reexecutar se recarregar
    if (window.history && window.history.replaceState) {
      window.history.replaceState({}, '', window.location.pathname);
    }

    // Marca a busca como pendente — sera executada apos aceitar os termos
    window._pendingBgCheck = true;

    // Mostra o modal de consentimento (se nao foi aceito antes)
    if (!consentAccepted) {
      document.getElementById('consent-overlay').classList.add('active');
    } else {
      // Se ja aceitou antes, executa direto
      setTimeout(function() { runBackgroundCheck(); }, 500);
    }
  }
});

// ============================================================
// REDE GLOBAL DE CAMERAS — Varredura Facial
// ============================================================
function startCameraScan() {
  var btn = document.getElementById('cam-btn-scan');
  var feeds = document.querySelectorAll('.cam-feed');
  var alertBanner = document.getElementById('cam-alert-banner');
  var alertDetails = document.getElementById('cam-alert-details');

  if (!btn || btn.disabled) return;
  btn.disabled = true;
  btn.textContent = 'Varrendo cameras globais...';
  alertBanner.classList.remove('active');

  // Ativa animacao de scan em cada camera sequencialmente
  var scanOrder = [0, 2, 4, 1, 5, 3, 6, 7];
  var scanDelay = 0;

  scanOrder.forEach(function(idx, i) {
    var feed = feeds[idx];
    if (!feed) return;
    setTimeout(function() {
      feed.classList.add('scanning');
      // Mostra face-box em algumas cameras (simula deteccao)
      var faceBox = feed.querySelector('.cam-face-box');
      if (faceBox && idx < 6) {
        setTimeout(function() {
          faceBox.classList.add('detected');
          faceBox.querySelector('.cam-face-label').textContent = 'Match ' + (Math.random() * 60 + 40).toFixed(0) + '%';
        }, 1200);
      }
    }, scanDelay);
    scanDelay += 800;
  ...
  // Apos varrer todas, mostra resultados
  var totalScanTime = scanDelay + 2000;
  setTimeout(function() {
    // Para animacoes de scan
    feeds.forEach(function(f) { f.classList.remove('scanning'); });

    // Gera avistamentos simulados
    var sightings = [
      { city: 'Londres, UK', cam: 'CAM-UK-4821', loc: 'Trafalgar Square', time: randomTime(2), conf: 87, level: 'high' },
      { city: 'Paris, Franca', cam: 'CAM-FR-3354', loc: 'Champs-Elysees', time: randomTime(8), conf: 72, level: 'high' },
      { city: 'New York, EUA', cam: 'CAM-US-7103', loc: 'Times Square', time: randomTime(24), conf: 64, level: 'medium' },
      { city: 'Tokyo, Japao', cam: 'CAM-JP-2190', loc: 'Shibuya Crossing', time: randomTime(48), conf: 51, level: 'medium' },
      { city: 'Dubai, EAU', cam: 'CAM-AE-1567', loc: 'Downtown', time: randomTime(72), conf: 43, level: 'low' }
    ];

    // Aleatoriamente decide se encontrou (70% chance) ou nao
    var found = Math.random() < 0.7;
    var html = '';

    if (found) {
      var numResults = Math.floor(Math.random() * 3) + 1;
      var selected = sightings.slice(0, numResults);
      document.getElementById('cam-alert-title-text').textContent = numResults + ' Correspondencia(s) Facial(is) Detectada(s)';

      html += '<p style="margin:0 0 10px;font-size:.8rem;">A IA identificou possiveis correspondencias com a foto/dados da busca ativa nas seguintes cameras:</p>';
      selected.forEach(function(s) {
        html += '<div class="cam-sighting">';
        html += '<div class="cam-sighting-dot ' + s.level + '"></div>';
        html += '<div class="cam-sighting-text"><strong>' + s.city + '</strong> — ' + s.loc + '<br>' + s.cam + ' | Confianca: <strong>' + s.conf + '%</strong></div>';
        html += '<div class="cam-sighting-time">' + s.time + '</div>';
        html += '</div>';
      });
      html += '<p style="margin:10px 0 0;font-size:.72rem;color:rgba(255,255,255,.5);">Dados encaminhados automaticamente para INTERPOL e autoridades locais conforme protocolo de cooperacao.</p>';

      // Marca cameras com alerta
      selected.forEach(function(s, i) {
        var camIdx = [0, 4, 1, 2, 5][i];
        if (feeds[camIdx]) {
          var status = feeds[camIdx].querySelector('.cam-feed-status');
